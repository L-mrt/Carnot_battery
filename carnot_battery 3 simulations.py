...
class CarnotBattery:
    ...
    def calc_orc_cycle(self) -> Tuple[float, float]:
        ...
        """
        # Evaporation temperature driven by hot tank temperature
...
def plot_3d_cop_hp(df: pd.DataFrame, config: Config):
    """
    3D plot of Heat Pump COP.
    COP depends only on T_hot and T_source (NOT on T_cold or T_amb).
    X=T_hot, Y=T_source, Z=COP — single uniform color, one subplot.
    Points are deduplicated on (T_hot, T_source) before plotting.
    """
    print(f"{'='*70}")
    print("3D VISUALIZATION - Heat Pump COP")
    print(f"{'='*70}")

    # Deduplicate: COP is identical for all (T_cold, T_amb) at the same (T_hot, T_source)
    data = (
        df[['T_hot', 'T_source', 'COP_HP']]
        .dropna(subset=['COP_HP'])
        .drop_duplicates(subset=['T_hot', 'T_source'])
    )
    ...
    ax.set_ylabel('T_source [°C]', fontsize=10, labelpad=8)
    ax.set_zlabel('COP [-]', fontsize=10, labelpad=8)
...
if __name__ == "__main__":
    ...
    # Visualize COP of Heat Pump (depends only on T_hot and T_source)
    plot_3d_cop_hp(df_results, config)

    # Visualize ORC efficiency (depends only on T_hot, T_cold, T_amb)
    plot_3d_eta_orc(df_results, config)
